...
Each entry is a deep copy — mutating fields (including `Tags` slices) does not
affect internal scheduler state.
...
deep copy — mutating its fields (including the `Tags` slice) does not affect
